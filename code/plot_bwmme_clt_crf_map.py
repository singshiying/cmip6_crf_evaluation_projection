...
t5=data_out_xr.loc[4,:,:,bmme].mean('models')-data_out_xr.loc[4,:,:,'OBS']
t6=data_out_xr.loc[4,:,:,wmme].mean('models')-data_out_xr.loc[4,:,:,'OBS']
...
position = fig.add_axes([0.92, 0.4, 0.010, 0.48])#位置[左,下,宽,高]
cb=fig.colorbar(c2,shrink=0.8,cax=position,fraction=0.03, format='% 1.0f')
cb.ax.tick_params(labelsize=9)
cb.set_label('W m$^{-2}$', fontsize=9,rotation=270)
position = fig.add_axes([0.92, 0.1, 0.010, 0.23])#位置[左,下,宽,高]
cb=fig.colorbar(c6,cax=position,shrink=0.8,fraction=0.03, format='% 1.0f')
cb.ax.tick_params(labelsize=9)
cb.set_label('%', fontsize=9,rotation=270)
plt.savefig('../fig/bwmme_clt_crf_map.svg',bbox_inches='tight')
...
c7=draw(fig,axes[2][0],7,'(h) SFC SWCRF,CERES ',clev,data_out[0,:,:,20]-data_out[2,:,:,20])
c8=draw(fig,axes[2][1],8,'(i) SFC LWCRF,CERES ',clev,data_out[1,:,:,20]-data_out[3,:,:,20])
c9=draw(fig,axes[2][2],9,'(j) SFC NETCRF,CERES ',clev,data_out[0,:,:,20]-data_out[2,:,:,20]+data_out[1,:,:,20]-data_out[3,:,:,20])
...
plt.savefig('../fig3/crf_obs_map.svg',bbox_inches='tight')
...
c7=draw(fig,axes[2][0],7,'(h) SFC SWCRF,Bias ',clev,model_mean[0,:,:]-model_mean[2,:,:]-(data_out[0,:,:,20]-data_out[2,:,:,20]))
c8=draw(fig,axes[2][1],8,'(i) SFC LWCRF,Bias ',clev,model_mean[1,:,:]-model_mean[3,:,:]-(data_out[1,:,:,20]-data_out[3,:,:,20]))
c9=draw(fig,axes[2][2],9,'(j) SFC NETCRF,Bias ',clev,model_mean[0,:,:]-model_mean[2,:,:]+model_mean[1,:,:]-model_mean[3,:,:]-(data_out[0,:,:,20]-data_out[2,:,:,20]+data_out[1,:,:,20]-data_out[3,:,:,20]))
position = fig.add_axes([0.92, 0.1, 0.010, 0.8])
cb=fig.colorbar(c3,shrink=0.8,cax=position,fraction=0.03, format='% 1.0f')
cb.ax.tick_params(labelsize=9)
cb.set_label('W m$^{-2}$', fontsize=9,rotation=270)
plt.savefig('../fig3/crf_bias_map.svg',bbox_inches='tight')
...
c4=draw(fig,axes[1][0],4,'(d) ATM SWCRF,AMME ',clev,model_mean[2,:,:])
c5=draw(fig,axes[1][1],5,'(e) ATM LWCRF,AMME ',clev,model_mean[3,:,:])
c6=draw(fig,axes[1][2],6,'(f) ATM NETCRF,AMME ',clev,model_mean[2,:,:]+model_mean[3,:,:])
c7=draw(fig,axes[2][0],7,'(h) SFC SWCRF,AMME ',clev,model_mean[0,:,:]-model_mean[2,:,:])
c8=draw(fig,axes[2][1],8,'(i) SFC LWCRF,AMME ',clev,model_mean[1,:,:]-model_mean[3,:,:])
c9=draw(fig,axes[2][2],9,'(j) SFC NETCRF,AMME ',clev,model_mean[0,:,:]-model_mean[2,:,:]+model_mean[1,:,:]-model_mean[3,:,:])

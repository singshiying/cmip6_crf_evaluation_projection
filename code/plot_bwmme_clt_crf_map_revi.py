...
data_out=np.load('/data04/shiy/code_cloud/code_paper_7.13/code2//fig1.npy')#5x180x360x21(20models,obs) 读取历史实验平均
...
width_cm = 15
height_cm = 14  # 保持原高度比例
fig = plt.figure(figsize=(width_cm / 2.54, height_cm / 2.54), dpi=600)
axes=fig.subplots(3,3,subplot_kw={'projection':ccrs.PlateCarree(central_longitude=180)})

fig.subplots_adjust(wspace=0.05,hspace=0.27,right=0.9,top=0.9,bottom=0.1)
clev=np.arange(-50,51,5)
c1=draw(fig,axes[0][0],1,'(a) TOA NETCRF,\n     BMME Bias',clev,t1)
c2=draw(fig,axes[0][1],2,'(b) TOA NETCRF,\n     WMME Bias',clev,t2)
c3=draw(fig,axes[0][2],2,'(c) TOA NETCRF,\n     WMME Bias-BMME Bias',clev,t2-t1,toanet_p)
c3=draw(fig,axes[1][0],3,'(d) ATM NETCRF,\n     BMME Bias',clev,t3)
c4=draw(fig,axes[1][1],4,'(e) ATM NETCRF,\n     WMME Bias',clev,t4)
c4=draw(fig,axes[1][2],2,'(f) ATM NETCRF,\n     WMME Bias-BMME Bias',clev,t4-t3,atmnet_p)
c5=draw(fig,axes[2][0],5,'(g) SFC NETCRF,\n     BMME Bias',clev,t5)
c6=draw(fig,axes[2][1],6,'(h) SFC NETCRF,\n     WMME Bias',clev,t6)
c2=draw(fig,axes[2][2],6,'(i) SFC NETCRF,\n     WMME Bias-BMME Bias',clev,t6-t5,sfcnet_p)
...
plt.savefig('../fig/bwmme_crf_map.pdf',bbox_inches='tight')
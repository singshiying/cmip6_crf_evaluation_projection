...
def draw_bar(ax,a,title,num):
    width=0.15
    rects1 = ax.bar(x - 2*width, a[0,:], width, label='TOA SWCRF',color='aqua')
    rects2 = ax.bar(x - width, a[1,:], width, label='TOA LWCRF',color='salmon')
    rects3 = ax.bar(x , a[2,:], width,label='SFC SWCRF',color='darkblue')
    rects4 = ax.bar(x+ width , a[3,:], width,label='SFC LWCRF',color='darkred')
    ...
    if num==1:
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(models,rotation=-270,ha='center',va='center_baseline',fontsize=9)
...
plt.savefig('../fig/variability_index.svg',bbox_inches='tight')
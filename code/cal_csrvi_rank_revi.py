#cal sfc and toa 4 varscsrvi rank score
import os
import numpy as np
import xarray as xr
import glob
...
def cal_csr(refsample1,sample1):
    ...
    corr = 1-df.corr().values#smaller better
    theta = np.arccos(corr[0,1])
    std = np.square(np.nanstd(sample1)-1)
    rmse =np.sqrt(np.nanmean(np.square(np.subtract(refsample1,sample1))))
    # print(np.sqrt(np.square(np.subtract(refsample,sample)).mean()))
    return corr[0,1],std,rmse

csri=np.zeros((21,4,5))#20model+obsx5varx5metric
for v in range(4):
    csri[:,v,3]=annual[v,:]
    csri[:,v,4]=season[v,:]
    for m in range(21):
        csri[m,v,0:3]=cal_csr(data_out2[v,:,20],data_out2[v,:,m])
    csri[20,v,0:3]=cal_csr(data_out2[v,:,20],np.mean(data_out2[v,:,0:20],1))
print(csri)
print('----')
csri=csri.reshape(21,20,order='A')
print(csri)
print(len(np.min(csri,0)))
###normalize
csri=(csri-np.min(csri,0))/(np.max(csri,0)-np.min(csri,0))#0-1
csri2=np.zeros((21,28))
for v in range(4):
    csri2[:,7*v:(7*v+3)]=csri[:,5*v:(5*v+3)]
    csri2[:,7*v+3]=np.mean(csri[:,5*v:(5*v+3)],1)
    csri2[:,(7*v+4):(7*v+6)]=csri[:,(5*v+3):(5*v+5)]
    csri2[:,7*v+6]=np.mean(csri[:,(5*v+3):(5*v+5)],1)
print(csri2)
...
# print('----')
# print(np.mean(csr_rank[:,[3,6,10,13,17,20,24,27]],1))
# #####small to large
...
csr_rank_xr=xr.DataArray(csri2,coords=[models,np.arange(0,28,1)],dims=['models','4varx7metric'])
csr_rank_xr.to_netcdf('./csr_vi_rank_score_21x28_revi.nc')
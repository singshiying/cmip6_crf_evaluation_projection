...
data_cas=xr.open_dataarray('/data04/shiy/code_cloud/code_paper_7.13/code2/ssp585_crf_clt_cas-esm2.nc')[:,0:1020]
print(data_out3.shape)
data_out3[0:5,:,5]=data_cas.values
...
t1[14:,4]=t1[14:,4]-t1[14,4]
t2[14:,4]=t2[14:,4]-t2[14,4]
t3[14:,4]=t3[14:,4]-t3[14,4]
t4[14:,4]=t4[14:,4]-t4[14,4]

# 创建2x2子图布局：宽度15cm（约5.9英寸），高度适配
fig_width_cm = 15
fig_width_inch = fig_width_cm / 2.54  # 厘米转英寸
fig_height_inch = fig_width_inch * 0.8  # 保持宽高比
fig, axes = plt.subplots(2, 2, figsize=(fig_width_inch, fig_height_inch), dpi=500)
fig.subplots_adjust(hspace=0.4, wspace=0.15, bottom=0.25)  # 调整间距，为底部图例留出空间
ax=axes.flatten()
...
draw3_range(ax[1],t1-t3,(t1-t3).loc[:,list(set(bmme))],(t1-t3).loc[:,list(set(wmme))],t2-t4,(t2-t4).loc[:,list(set(bmme))],(t2-t4).loc[:,list(set(wmme))],obs2[0,:]+obs2[1,:]-(obs2[2,:]+obs2[3,:]),'b) ATM NETCRF',2)

draw3_range(ax[2],t3,t3.loc[:,list(set(bmme))],t3.loc[:,list(set(wmme))],t4,t4.loc[:,list(set(bmme))],t4.loc[:,list(set(wmme))],obs2[2,:]+obs2[3,:],'c) SFC NETCRF',2)
...
fig.legend(handles, labels, 
          ...
          bbox_to_anchor=(0.5, 0.02),  # 精确位置
          ncol=3,  # 分4列显示，避免过长
          frameon=True,  # 显示边框
          fontsize=9,
          columnspacing=1.0,
          handlelength=1.5)
plt.savefig('../fig/bwmme_ssp_trend.pdf',bbox_inches='tight')

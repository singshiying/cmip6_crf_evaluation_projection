...
c3=draw(fig,axes[0][2],3,'(c) TOA NETCRF,AMME    W m$^{-2}$',clev,model_mean[0,:,:]+model_mean[1,:,:])
c4=draw(fig,axes[1][0],4,'(d) TOA SWCRF,CERES ',clev,data_out[0,:,:,20])
c5=draw(fig,axes[1][1],5,'(e) TOA LWCRF,CERES ',clev,data_out[1,:,:,20])
c6=draw(fig,axes[1][2],6,'(f) TOA NETCRF,CERES ',clev,data_out[0,:,:,20]+data_out[1,:,:,20])
clev=np.arange(-20,21,5)
c7=draw(fig,axes[2][0],7,'(h) TOA SWCRF,Bias ',clev,model_mean[0,:,:]-data_out[0,:,:,20])
c8=draw(fig,axes[2][1],8,'(i) TOA LWCRF,Bias ',clev,model_mean[1,:,:]-data_out[1,:,:,20])
c9=draw(fig,axes[2][2],9,'(j) TOA NETCRF,Bias ',clev,model_mean[0,:,:]+model_mean[1,:,:]-data_out[0,:,:,20]-data_out[1,:,:,20])
position = fig.add_axes([0.92, 0.59, 0.010, 0.19])#位置[左,下,宽,高]
...
position = fig.add_axes([0.92, 0.35, 0.010, 0.19])#位置[左,下,宽,高]
cb=fig.colorbar(c6,cax=position,shrink=0.8,fraction=0.03, format='% 1.0f')
position = fig.add_axes([0.92, 0.11, 0.010, 0.19])#位置[左,下,宽,高]
cb.ax.tick_params(labelsize=9)
...
cb.ax.tick_params(labelsize=9)
cb.set_ticks([ -40,-30,-20,-10,10,0,10,20,30,40])
# cb.set_ticklabels([-40,0,40])

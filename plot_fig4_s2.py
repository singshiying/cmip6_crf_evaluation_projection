...
season1=np.load('fig4_s2.npy')#.reshape(5,180*360,21,order='A')#5var,latxlon,20model+AMMME
annual1=np.load('fig4_s3.npy')#.reshape(5,180*360,21,order='A')
annual1[:,:,:,20]=np.mean(annual1[0,:,:,0:20],2)
###AVI AMME用多模式平均 SVI先计算多模式平均再计算VI
...
######################################
####csr_rank是最后呈现的结果
csr_rank=np.zeros([21,25])#5x5
for v in range(5):
    csr_rank[:,5*v:(5*v+3)]=csr_rank1[:,5*v:(5*v+3)]####csr
    rank=np.zeros(21)
    temp=np.mean(csr_rank1[:,5*v:(5*v+3)],1)####csr 得分平均
    index=np.argsort(temp)#从小到大排序
    rank[index]=np.arange(1,22)
    csr_rank[:,5*v+3]=rank####第四列
    csr_rank[:,5*v+4]=csr_rank1[:,5*v+4]#取出SVI

print('----')
# print(np.mean(csr_rank[:,[3,6,10,13,17,20,24,27,31,34]],1))
print(np.mean(csr_rank[:,[3,4,8,9,13,14,18,19,23,24]],1))
#####依据总分数从小到大排序
# index1=np.argsort(np.mean(csr_rank[:,[3,6,10,13,17,20,24,27,31,34]],1))
index1=np.argsort(np.mean(csr_rank[:,[3,4,8,9,13,14,18,19,23,24]],1))
models_p1=models
models_p1=np.array(models)[index1]
csr_rank_xr=xr.DataArray(csr_rank,coords=[models,np.arange(0,25,1)],dims=['models','5varx5metric'])
csr_rank_xr.to_netcdf('./csr_svi_rank_score_21x25.nc')
#############csr_rank_p1为plot的rank score
...
ax.xaxis.set_major_locator(FixedLocator(np.arange(0,25,5)))#设置y主坐标间隔 1
...
xlabel=['c','s','r','csr','svi']*5
ax.set_xticks(np.arange(0.5,25,1),minor=True)
ax.set_xticklabels(xlabel,minor=True)
# ax.set_title("a) Toa net crf",loc='left',pad=10)
ax.text(2.5, 26.5-4,"TOA SWCRE",ha='center',va='center',fontsize=14,rotation=0)
ax.text(7.5, 26.5-4,"TOA LWCRE",ha='center',va='center',fontsize=14,rotation=0)
ax.text(12.5, 26.5-4,"SFC SWCRE",ha='center',va='center',fontsize=14,rotation=0)
ax.text(17.5, 26.5-4,"SFC LWCRE",ha='center',va='center',fontsize=14,rotation=0)
ax.text(22.5, 26.5-4,"CLT",ha='center',va='center',fontsize=14,rotation=0)

plt.savefig('fig4_s2.png',bbox_inches='tight')

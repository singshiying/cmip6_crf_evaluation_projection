...
ssp585_ts_xr=xr.DataArray(data_out3,coords=[np.arange(0,5,1),time],dims=['vars','time'])


toa_netcrf_yearly = (ssp585_ts_xr.loc[2,:]+ssp585_ts_xr.loc[3,:]).groupby('time.year').mean()
...
plt.title('SFC Net Cloud Radiative Forcing - CAS-ESM2 SSP585')
plt.grid(True)

# Save figure
plt.savefig('../fig/ssp585_sfc_netcrf_yearly_cas-esm2z.png', dpi=300, bbox_inches='tight')
plt.close()

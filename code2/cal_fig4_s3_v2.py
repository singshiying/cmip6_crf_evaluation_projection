#计算不同模式180x360 每个格点的年平均数据VI
############5varx180latx360lonx20model
import os
import numpy as np
import xarray as xr
import glob
...
data_out=np.zeros((5,14,180,360,len(models)+1))#toa cre,sfc cre,clt;models+cas+obs
...
sw_crf_weighted_mean=sw_crf.groupby('time.year').mean()#14x180x360
...
lw_crf_weighted_mean=lw_crf.groupby('time.year').mean()
...
data_out[2,:,:,:,4]=sw_crf_weighted_mean.values-sfc_sw_crf.groupby('time.year').mean().values
#求sfc lw crf
#rlds-rldscs
...
data_out[3,:,:,:,4]=lw_crf_weighted_mean.values-sfc_lw_crf.groupby('time.year').mean().values
#求clt
...
data_out[4,:,:,:,4]=clt.groupby('time.year').mean().values
for m in range(len(models)):
    if m!=4:
        ...
        sw_crf=sw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        # sw_crf.data=sw_crf.data/time_lat_lon_weight
        data_out[0,:,:,:,m]=sw_crf.groupby('time.year').mean().values
        #求lwcrf
        ...
        lw_crf=rlutcs-rlut
        lw_crf=lw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[1,:,:,:,m]=lw_crf.groupby('time.year').mean().values
        #求sfc sw cre
        ...
        sfc_sw_crf=rsds-rsus-rsdscs+rsuscs
        sfc_sw_crf=sfc_sw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[2,:,:,:,m]=sw_crf.groupby('time.year').mean().values-sfc_sw_crf.groupby('time.year').mean().values
        #求sfc lw cre
        ...
        sfc_lw_crf=rlds-rldscs
        sfc_lw_crf=sfc_lw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[3,:,:,:,m]=lw_crf.groupby('time.year').mean().values-sfc_lw_crf.groupby('time.year').mean().values
        #求clt
        ...
        clt=clt.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[4,:,:,:,m]=clt.groupby('time.year').mean().values
...
# sw_crf.data=sw_crf.data/time_lat_lon_weight
data_out[0,:,:,:,len(models)]=sw_crf.groupby('time.year').mean().values
lw_crf=fc['toa_cre_lw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon
data_out[1,:,:,:,len(models)]=lw_crf.groupby('time.year').mean().values
sfc_sw_crf=fc['sfc_cre_net_sw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon
data_out[2,:,:,:,len(models)]=sw_crf.groupby('time.year').mean().values-sfc_sw_crf.groupby('time.year').mean().values
sfc_lw_crf=fc['sfc_cre_net_lw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon
data_out[3,:,:,:,len(models)]=lw_crf.groupby('time.year').mean().values-sfc_lw_crf.groupby('time.year').mean().values
#读取ISCCP-h
...
data_out[4,:,:,:,len(models)]=clt.groupby('time.year').mean().values
...
def cal_VI2(ref,sample):
    #sample是20个模式 5d varxyearxlatxlonxmodels
    stdref=np.nanstd(ref[:,:,:,:],axis=1,ddof=1)
    # stdref2=np.repeat(stdref,sample.shape[4],axis=2).reshape(5,180,360,sample.shape[4])
    # print(stdref2[:,:,:,0])
    # print(stdref2[:,:,:,12])
...
da = os.path.exists('fig4_s3_v2.npy')
if da:
    os.remove('fig4_s3_v2.npy')

print(data_out2.shape)
np.save('fig4_s3_v2.npy',data_out2)
#计算models的历史实验与观测200101-201412多年平均的tcf,cre map
####与cal_fig3_s2,cal_fig3的区别在于模式个数，该文件是paper使用的20个模式
######5varx180latx360lonx20model+1obs
import os
import numpy as np
import xarray as xr
import glob
...
data_out=np.zeros((5,180,360,len(models)+1))#toa cre,sfc cre,clt;models+cas+obs
print('cas-esm2')
#cas-esm2
#求swcrf
...
time_len=len(sw_crf.coords['time'])
lon_len=len(olon)
lat_len=len(olat)
...
data_out[2,:,:,4]=sw_crf_weighted_mean.values-sfc_sw_crf.mean(('time')).values
#求sfc lw crf
#rlds-rldscs
...
data_out[3,:,:,4]=lw_crf_weighted_mean.values-sfc_lw_crf.mean(('time')).values
#求clt
...
for m in range(len(models)):
    if m!=4:
        ...
        sfc_sw_crf=rsds-rsus-rsdscs+rsuscs
        sfc_sw_crf=sfc_sw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[2,:,:,m]=sw_crf.mean(('time')).values-sfc_sw_crf.mean(('time')).values
        #求sfc lw cre
        ...
        sfc_lw_crf=rlds-rldscs
        sfc_lw_crf=sfc_lw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[3,:,:,m]=lw_crf.mean(('time')).values-sfc_lw_crf.mean(('time')).values
        #求clt
...
data_out[2,:,:,len(models)]=sw_crf.mean(('time')).values-sfc_sw_crf.mean(('time')).values
sfc_lw_crf=fc['sfc_cre_net_lw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon
data_out[3,:,:,len(models)]=lw_crf.mean(('time')).values-sfc_lw_crf.mean(('time')).values
#读取ISCCP-h
...
da = os.path.exists('fig1.npy')
if da:
    os.remove('fig1.npy')

print(data_out.shape)
np.save('fig1.npy',data_out)
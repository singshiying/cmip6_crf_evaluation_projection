#计算不同模式180x360 每个格点的年平均数据相比于观测的VI值
...
data_out[2,:,:,:,4]=sfc_sw_crf.groupby('time.year').mean().values
#求sfc lw crf
#rlds-rldscs
...
data_out[3,:,:,:,4]=sfc_lw_crf.groupby('time.year').mean().values
#求clt
...
for m in range(len(models)):
    if m!=4:
        ...
        sfc_sw_crf=rsds-rsus-rsdscs+rsuscs
        sfc_sw_crf=sfc_sw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[2,:,:,:,m]=sfc_sw_crf.groupby('time.year').mean().values
        #求sfc lw cre
        ...
        sfc_lw_crf=rlds-rldscs
        sfc_lw_crf=sfc_lw_crf.interp(lat=olat,lon=olon,kwargs={'fill_value': 'extrapolate'})
        data_out[3,:,:,:,m]=sfc_lw_crf.groupby('time.year').mean().values
        #求clt
...
data_out[2,:,:,:,len(models)]=sfc_sw_crf.groupby('time.year').mean().values
sfc_lw_crf=fc['sfc_cre_net_lw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon
data_out[3,:,:,:,len(models)]=sfc_lw_crf.groupby('time.year').mean().values
#读取ISCCP-h
...
data_out2[:,:,:,20]=cal_VI2(data_out[:,:,:,:,20],np.nanmean(data_out[:,:,:,:,0:20],4))
# data_out2[:,:,:,20]=np.nanmean(data_out2[:,:,:,0:20],3)
da = os.path.exists('fig4_s3.npy')
if da:
    os.remove('fig4_s3.npy')

print(data_out2.shape)
np.save('fig4_s3.npy',data_out2)
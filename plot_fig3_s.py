...
sfc_net=data_out2[2,:,:]+data_out2[3,:,:]
def set_tayloraxes(fig, location):
    ...
    ax.axis["top"].label.set_fontsize(24)
    ax.axis["left"].set_axis_direction("bottom") 
    ax.axis["left"].major_ticklabels.set_fontsize(20)
    ax.axis["left"].label.set_text("Standard deviation")
    ax.axis["left"].label.set_fontsize(24)
    ...
    r = np.zeros_like(t) + 1
    polar_ax.plot(t,r,'k--')
    polar_ax.text(np.pi/2+0.032,1.02, " 1.00", size=20,ha="right", va="top",
...
def plot_taylor(axes, refsample, sample,num, *args, **kwargs):
    ...
    # if num<24:
    #     t = axes.text(t,r,num+1,fontsize=14,color=kwargs['color'])
    return d
...
for m in np.arange(0,20):
    d1 = plot_taylor(ax1,data_out2[0,:,20],data_out2[0,:,m], m,color='aqua',marker='o',alpha=0.8,markersize=7,markeredgewidth=2,label=models[m])
    d2 = plot_taylor(ax1,data_out2[1,:,20],data_out2[1,:,m], m,color='salmon',marker='o',alpha=0.8,markersize=7,markeredgewidth=2,label=models[m])
    # d3 = plot_taylor(ax1,toa_net[:,24],toa_net[:,m], m,color='gold',marker='o',alpha=0.8,markersize=2,markeredgewidth=2,label=models[m])
    d4 = plot_taylor(ax1,data_out2[2,:,20],data_out2[2,:,m], m,color='darkblue',marker='o',alpha=0.8,markersize=7,markeredgewidth=2,label=models[m])
    d5 = plot_taylor(ax1,data_out2[3,:,20],data_out2[3,:,m], m,color='darkred',marker='o',alpha=0.8,markersize=7,markeredgewidth=2,label=models[m])
    # d6 = plot_taylor(ax1,sfc_net[:,24],sfc_net[:,m], m,color='olive',marker='o',alpha=0.8,markersize=2,markeredgewidth=2,label=models[m])
    d7 = plot_taylor(ax1,data_out2[4,:,20],data_out2[4,:,m], m,color='g',marker='o',alpha=0.8,markersize=7,markeredgewidth=2,label=models[m])
d8 = plot_taylor(ax1,data_out2[0,:,20],np.mean(data_out2[0,:,0:20],1), 24,color='aqua',marker='v',markeredgecolor='k',markeredgewidth=2,alpha=0.7,markersize=14,zorder=50)
d9 = plot_taylor(ax1,data_out2[1,:,20],np.mean(data_out2[1,:,0:20],1), 24,color='salmon',marker='v',markeredgecolor='k',markeredgewidth=2,alpha=0.7,markersize=14,zorder=50)
...
d11 = plot_taylor(ax1,data_out2[2,:,20],np.mean(data_out2[2,:,0:20],1), 24,color='darkblue',marker='v',markeredgecolor='k',markeredgewidth=2,alpha=0.7,markersize=14,zorder=50)
d12 = plot_taylor(ax1,data_out2[3,:,20],np.mean(data_out2[3,:,0:20],1), 24,color='darkred',marker='v',markeredgecolor='k',markeredgewidth=2,alpha=0.7,markersize=14,zorder=50)
...
d14 = plot_taylor(ax1,data_out2[4,:,20],np.mean(data_out2[4,:,0:20],1), 24,color='g',marker='v',markeredgecolor='k',markeredgewidth=2,alpha=0.7,markersize=14,zorder=50)

ax1.text(0.85,0.95,'Top of atmosphere shortwave radiative forcing(Toa sw crf)',color='aqua',transform=ax1.transAxes,fontsize=24)
ax1.text(0.85,0.89,'Top of atmosphere longwave radiative forcing(Toa lw crf)',color='salmon',transform=ax1.transAxes,fontsize=24)
# ax1.text(0.85,0.85,'Top of atmosphere net radiative forcing(NETCRE TOA)',color='gold',transform=ax1.transAxes,fontsize=14)
ax1.text(0.85,0.83,'Surface shortwave radiative forcing(Sfc sw crf)',color='darkblue',transform=ax1.transAxes,fontsize=24)
ax1.text(0.85,0.77,'Surface longwave radiative forcing(Sfc lw crf)',color='darkred',transform=ax1.transAxes,fontsize=24)
# ax1.text(0.85,0.7,'Surface net radiative forcing(NETCRE SFC)',color='olive',transform=ax1.transAxes,fontsize=14)
ax1.text(0.85,0.71,'Total cloud fraction(CLT)',color='g',transform=ax1.transAxes,fontsize=24)

plt.savefig('fig3_s.png',bbox_inches='tight')
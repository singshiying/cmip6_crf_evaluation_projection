#计算models的历史实验与观测200101-201412多年平均全球平均的tcf,cre
import os
import numpy as np
import xarray as xr
import glob
import pandas as pd
#取出his的crf,scrf,acrf,clt
...
modelsp=['ACCESS-CM2','BCC-CSM2-MR','CanESM5','CESM2-WACCM','CIESM','CMCC-CM2-SR5','FGOALS-f3-L','INM-CM4-8','MPI-ESM1-2-LR','MRI-ESM2-0','ACCESS-ESM1-5', 'AWI-CM-1-1-MR', 'E3SM-1-0', 'EC-Earth3-CC', 'FGOALS-g3', 'GFDL-ESM4',  'INM-CM5-0', 'IPSL-CM6A-LR', 'KACE-1-0-G', 'KIOST-ESM', 'MIROC6', 'MPI-ESM1-2-HR', 'NESM3','CAS-ESM2-0','Obs']
model_his='/data04/shiy/cmip6_his_ssp585/historical/'
model_ssp585='/data04/shiy/cmip6_his_ssp585/ssp585/'
data_out=np.zeros((5,len(models)+2))#toa cre,sfc cre,clt;models+cas+obs
#cas-esm2
#求swcrf
...
#将短波辐射云强迫都矫正为太阳直射时的swcrf，以去除太阳入射角度对swcrf的影响，全部除以cos（lat）
# weights=np.cos(np.deg2rad(sw_crf.coords['lat']))*np.cos(np.deg2rad(sw_crf.coords['lat']))
# weights.name='weights'
# sw_crf_weighted=sw_crf.weighted(weights)
sw_crf_weighted_mean=sw_crf.mean(('lon','lat','time'))#一个数
# print(sw_crf_weighted_mean)
data_out[0,len(models)]=sw_crf_weighted_mean.values
#求lwcrf
...
lw_crf_weighted_mean=lw_crf_weighted.mean(('lon','lat','time'))
data_out[1,len(models)]=lw_crf_weighted_mean.values
#求sfc sw crf
...
sfc_sw_crf_weighted_mean=sfc_sw_crf_weighted.mean(('lon','lat','time'))
data_out[2,len(models)]=sfc_sw_crf_weighted_mean.values
#求sfc lw crf
#rlds-rldscs
...
sfc_lw_crf_weighted_mean=sfc_lw_crf_weighted.mean(('lon','lat','time'))
data_out[3,len(models)]=sfc_lw_crf_weighted_mean.values
#求clt
...
weights2=np.cos(np.deg2rad(lw_crf.coords['lat']))
clt_weighted=clt.weighted(weights2)
clt_weighted_mean=clt_weighted.mean(('lon','lat','time'))
data_out[4,len(models)]=clt_weighted_mean.values
...
for m in range(len(models)):
    ...
    # weights.name='weights'
    # sw_crf_weighted=sw_crf.weighted(weights)
    sw_crf_weighted_mean=sw_crf.mean(('lon','lat','time'))
    ...
    sfc_sw_crf_weighted=sfc_sw_crf.weighted(weights2)
    sfc_sw_crf_weighted_mean=sfc_sw_crf_weighted.mean(('lon','lat','time'))
    data_out[2,m]=sfc_sw_crf_weighted_mean.values
    #求sfc lw cre
    ...
    sfc_lw_crf_weighted=sfc_lw_crf.weighted(weights2)
    sfc_lw_crf_weighted_mean=sfc_lw_crf_weighted.mean(('lon','lat','time'))
    data_out[3,m]=sfc_lw_crf_weighted_mean.values
    #求clt
    ...
    # print(weights2)
    weights2=np.cos(np.deg2rad(clt.coords['lat']))
    # print(weights2)
    clt_weighted=clt.weighted(weights2)
    clt_weighted_mean=clt_weighted.mean(('lon','lat','time'))
    # print(clt_weighted.mean(('lon','lat','time')).values)
    data_out[4,m]=clt_weighted_mean.values
...
sw_crf=fc['toa_cre_sw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon取北半球
sw_crf_weighted_mean=sw_crf.mean(('lon','lat','time'))
data_out[0,len(models)+1]=sw_crf_weighted_mean
lw_crf=fc['toa_cre_lw_mon'].loc['2001-01-01':'2014-12-30',-90:90,0:360]#time,lat,lon取北半球
weights2=np.cos(np.deg2rad(lw_crf.coords['lat']))
lw_crf_weighted=lw_crf.weighted(weights2)
lw_crf_weighted_mean=lw_crf_weighted.mean(('lon','lat','time'))
...
da = os.path.exists('tabel1.txt')
if da:
    os.remove('tabel1.txt')

output = open('tabel1','w+')
for j in range(data_out.shape[1]):
    output.write(modelsp[j])
    output.write(' ')
    for i in range(data_out.shape[0]):
        output.write(str(round(data_out[i][j],2)))
        output.write(' ')
    output.write('\n') 

...
def cal_my_ssp585(num):
    ...
    if num==0 or num==1:
        ...
        cas_path=cas_path+os.listdir(cas_path)[0]+'/'
        cas_path=cas_path+os.listdir(cas_path)[0]
        clt=xr.open_dataset(cas_path)['tas'].loc['2015-01-01':'2099-12-31',int(area_my[0]):int(area_my[1]),0:360] #240x48x192
        weights=np.cos(np.deg2rad(clt.coords['lat']))
        clt_weighted=clt.weighted(weights)
        clt_weighted_mean=clt_weighted.mean(('lon','lat'))
        data_out[5,:,5]=clt_weighted_mean
        for m in range(len(models)):
            if m!=5:
                ...
                clt_weighted_mean=clt_weighted.mean(('lon','lat'))
                #annual_clt=clt_weighted_mean.groupby('time.season').mean()
                data_out[5,:,m]=clt_weighted_mean
    else:
        #先取出cas的历史实验各个变量并求areamean
        print('cas-esm2')
        cas_his='/data03/cmip6/post-new/CMIP6/CMIP6/ScenarioMIP/CAS/CAS-ESM2-0/ssp585/r1i1p1f1/Amon/rsutcs/'
...
final_out3=np.zeros((6,1020,len(models)))
final_out3[:,:,:]=cal_my_ssp585(0)
da = os.path.exists('fig7_3.npy')
if da:
    os.remove('fig7_3.npy')
np.save('fig7_3.npy',final_out3)

print(final_out3.shape,'file save')

...
def cal_my_his(num):
    ...
    if num==0 or num==1:
        ...
        cas_path=cas_path+os.listdir(cas_path)[0]+'/'
        cas_path=cas_path+os.listdir(cas_path)[0]
        tas=xr.open_dataset(cas_path)['tas'].loc['1850-01-01':'2014-12-30',int(area_my[0]):int(area_my[1]),0:360] #240x48x192
        ...
        for m in range(len(models)):
            if m!=5:
                ...
                tas=xr.open_dataset(fhis)['tas'].loc['1850-01-01':'2014-12-30',int(area_my[0]):int(area_my[1]),0:360] #240x48x192
                weights=np.cos(np.deg2rad(tas.coords['lat']))
                tas_weighted=tas.weighted(weights)
                tas_weighted_mean=tas_weighted.mean(('lon','lat'))
        ...
        weights1=np.cos(np.deg2rad(sw_crf.coords['lat']))#对纬度赋予权重求加权平均
        weights1.name='weights'
        weights2=np.cos(np.deg2rad(sw_crf2.coords['lat']))#对纬度赋予权重求加权平均
...
final_out1=np.zeros((6,1980,len(models)))
final_out1[:,:,:]=cal_my_his(0)
